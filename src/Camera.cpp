...
namespace raytracer {
Camera::Camera(Vector origin, Vector target, Vector upguide, double fov,
               double aspectRatio)
    : _position{origin} {
  target = (origin + target).normalize();
  upguide = (origin + upguide).normalize();
...
Vector Camera::position() const { return _position; }

void Camera::setPosition(const Vector &position, const Vector &upguide,
...
Vector Camera::direction() const { return _forward; }
